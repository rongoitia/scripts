...
if [ $TOTCAPSSD == 0 ]
then
echo "Total SSD Disk Capacity = 0"
else
...
then
echo $VMFS : CRITICAL! TOTAL: $VSIZE\Mb\ USED: $USED\Mb\ \($USED-PERCENT%\) FREE: $FREE\Mb\ \($FREE-PERCENT\) TotSize=$VSIZE_Gb\Gb\ Used=$USED_Gb\Gb\,\Warn Limit Used :\ $WARNCRAW_Gb \Gb\,\Crit Limit Used :\ $CRITCCRAW_Gb\Gb
#rm -f $TMPDIR/3par_$COMMAND.$INSERV.out
#exit 2
else
if [ $USED_Gb -ge $WARNCRAW_Gb ]
then
echo $VMFS : WARNING! TOTAL: $VSIZE\Mb\ USED: $USED\Mb\ \($USED-PERCENT%\) FREE: $FREE\Mb\ \($FREE-PERCENT\) TotSize=$VSIZE_Gb\Gb\ Used=$USED_Gb\Gb\,\Warn Limit Used :\ $WARNCRAW_Gb\Gb\,\Crit Limit Used :\ $CRITCCRAW_Gb\Gb
#rm -f $TMPDIR/3par_$COMMAND.$INSERV.out
#exit 1
else

echo $VMFS : OK : TOTAL: $VSIZE\Mb\ USED: $USED\Mb\ \($USEDPERCENT%\) FREE: $FREE\Mb\ \($FREEPERCENT%\) TotSize=$VSIZE_Gb\Gb\ Used=$USED_Gb\Gb\,\Warn Limit Used :\ $WARNCRAW_Gb\Gb\,\Crit Limit Used :\ $CRITCCRAW_Gb\Gb
#rm -f $TMPDIR/3par_$COMMAND.$INSERV.out
#exit 0
fi
fi
